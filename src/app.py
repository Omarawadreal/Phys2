...
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///' + os.path.join(os.path.dirname(os.path.dirname(__file__)), 'physics.db')
db = SQLAlchemy(app)
...
if __name__ == '__main__':
    with app.app_context():
        ...
        stu1 = User(utorId='student1', password=bcrypt.generate_password_hash('student1').decode('utf-8'), studentId=1000000001, displayName='Student1', email='student1@psut.edu.jo', accountType='stu', createdAt=int(time.time()))
        stu2 = User(utorId='student2', password=bcrypt.generate_password_hash('student2').decode('utf-8'), studentId=1000000002, displayName='Student2', email='student2@psut.edu.jo', accountType='stu', createdAt=int(time.time()))
        ins1 = User(utorId='instructor1', password=bcrypt.generate_password_hash('instructor1').decode('utf-8'), displayName='Instructor1', email='instructor1@psut.edu.jo', accountType='ins', createdAt=int(time.time()))
        ins2 = User(utorId='instructor2', password=bcrypt.generate_password_hash('instructor2').decode('utf-8'), displayName='Instructor2', email='instructor2@psut.edu.jo', accountType='ins', createdAt=int(time.time()))
        if not User.query.first():
            db.session.add_all([stu1, stu2, ins1, ins2])
            db.session.commit()
        
        if not MarkGroup.query.first():
            quiz1 = MarkGroup(title='Quiz 1', maxGrade=100, 
                                  createdAt=int(time.time()), 
                                  releasedAt=int(time.time()) - 86400 * 3)
            quiz2 = MarkGroup(title='Quiz 2', maxGrade=100, 
                                  createdAt=int(time.time()), 
                                  releasedAt=int(time.time()) - 86400 * 3)
            quiz3 = MarkGroup(title='Quiz 3', maxGrade=100, 
                                  createdAt=int(time.time()), 
                                  releasedAt=int(time.time()) + 86400 * 3)
            quiz4 = MarkGroup(title='Quiz 4', maxGrade=100, 
                                  createdAt=int(time.time()), 
                                  releasedAt=int(time.time()) + 86400 * 3)
            quiz5 = MarkGroup(title='Quiz 5', maxGrade=10, 
                                  createdAt=int(time.time()), 
                                  releasedAt=int(time.time()) - 86400 * 3)
            tutorial1 = MarkGroup(title='Lab 1', maxGrade=10, 
                                  createdAt=int(time.time()), 
                                  releasedAt=int(time.time()) - 86400 * 3)
            tutorial2 = MarkGroup(title='Lab 2', maxGrade=10, 
                                  createdAt=int(time.time()), 
                                  releasedAt=int(time.time()) - 86400 * 3)
            tutorial3 = MarkGroup(title='Lab 3', maxGrade=10, 
                                  createdAt=int(time.time()), 
                                  releasedAt=int(time.time()) - 86400 * 3)
